...
def make_map():
    ...
    map_html=f'''<div class="map-tool"><div class="map-toolbar"><span class="eyebrow">OUR LITTLE ROAD TRIP</span>{filters}</div><div class="map-layout"><div class="map-surface" tabindex="0" role="region" aria-label="福井県の旅の地図。拡大時は左右にスクロールできます"><button class="map-zoom" id="map-zoom" type="button" aria-pressed="false">地図を拡大 ＋</button>{svg}<div class="map-legend">{legend}<span>→ 訪問順</span></div></div><aside class="map-story" aria-label="地図の案内"><p class="eyebrow" id="map-story-kicker">3 DAYS / ひとめぐり</p><h3 id="map-story-title">北へ、東へ。<br>ふたりの福井を結ぶ。</h3><p id="map-story-note">日付を選ぶと、その日の移動を表示します。地図の地点を押して、旅先の案内を開いてみて。</p><div id="map-itinerary"></div><div class="map-selection" id="map-selection" hidden><p id="map-location-name"></p><p id="map-location-note"></p><a id="map-location-link" href="#places">施設のくわしい案内 ↗</a></div><p class="map-finish" id="map-finish">東京 ↔ 福井は北陸新幹線<br>県内はレンタカー。夜の福井市内は徒歩で。</p></aside></div><p class="map-caption">地点は周辺の代表位置、線は訪問順を結ぶ概略です。実際の道路・駐車先は施設の経路ボタンで確認。<a href="credits.html#map-credits">地図の出典 ↗</a></p></div>'''
...
def make_summary(days):
    ...
    return f'''<section class="section-wrap summary-section" id="timetable"><div class="section-heading"><div><p class="eyebrow">THE WHOLE TRIP, AT A GLANCE</p><h2>三日間を、一枚に。</h2></div><p>朝の出発から、夜の一杯まで。<br>気になる予定を押すと、くわしい案内へ。</p></div><div class="summary-topline"><span>2026.09.21 — 23</span><span>● 固定の予約・列車　／　そのほかは計画上の目安</span></div><p class="summary-scroll-hint">左右にスワイプして、3日間を見比べる ↔</p><div class="summary-scroll" role="region" aria-label="全日程のタイムテーブル。横スクロールできます" tabindex="0"><table class="summary-table"><caption class="sr-only">2026年9月21日から23日の主要予定。朝・昼・午後・夜ごとに3日間を比較。</caption><thead><tr><th scope="col" class="summary-corner">TIME<br><span>OF DAY</span></th>{header}</tr></thead><tbody>{rows}</tbody></table></div><div class="summary-ticket"><span>RETURN TICKET</span><p><b>9.23　18:18 福井 → 21:20 東京</b><small>かがやき582号 · 指定席2名</small></p><a href="#day-3">最終日の詳細 ↗</a></div></section>'''
...
def make_stamps():
    buttons=''.join(f'<button class="travel-stamp" type="button" data-stamp="{id}" aria-pressed="false" aria-label="{title}のスタンプを押す"><span class="stamp-day">{day}</span><strong>{kanji}</strong><span class="stamp-word">{title}</span><span class="stamp-action">ぽん、と押す</span></button>' for id,kanji,title,day in STAMPS)
    return f'''<section class="stamp-section section-wrap" id="stamps"><div class="section-heading"><div><p class="eyebrow">SIX LITTLE SOUVENIRS</p><h2>思い出を、ぽん。</h2></div><p>訪れたら、ひと押し。<br>ふたりの旅に、六つのしるし。</p></div><div class="stamp-book">{buttons}</div><div class="stamp-bottom"><p id="stamp-message" role="status">まだ白紙のスタンプ帳。どこから思い出が増えるかな。</p><span id="stamp-count">0 / 6</span></div><p class="storage-hint">スタンプはこの端末に保存。もう一度押すと取り消せます。</p></section>'''
