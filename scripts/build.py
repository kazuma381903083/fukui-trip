...
days = [
 dict(n=1,date='2026-09-21',short='9.21',weekday='MON',jp='月',title='太古に出会い、糸を織り、<br>湯にほどける。',lead='恐竜の世界を歩き、自分の手で布を織る。<br>一日の終わりは、あわらの湯と会席に。',area='福井駅 → 勝山 → あわら温泉',stay='越前あわら温泉 長谷川',focus='15:00 手織り体験 ／ 17:30 旅館到着予定',note='恐竜博物館は連休の交通規制に注意。臨時駐車場の場合は13:00頃を目安に見学を終え、帰りのシャトルへ。',image='dinosaur.webp'),
 ...
 dict(n=3,date='2026-09-23',short='9.23',weekday='WED',jp='水',title='城下町の記憶と、<br>帰り際の一杯。',lead='一乗谷の暮らしに、ゆっくり思いを巡らす。<br>車を返したら、福井の酒でもう一度乾杯。',area='福井市 → 一乗谷 → 福井駅 → 東京',stay='18:18 福井発 → 21:20 東京着',focus='15:00 一乗谷出発 ／ 返却目標16:00〜16:30',note='一乗谷ガイドは予約調整中。手配できなければ同じ時間帯に自主見学。帰路が混んだら駅の角打ちを省略します。',image='ichijodani.webp'),
]
...
journey=make_journey(days, make_discoveries(places))

place_cards=''
for p in places:
    place_cards+=f'''<article class="place-card" id="place-{p['id']}" data-place-day="{p['day']}"><div class="place-meta"><span>DAY 0{p['day']} / {p['kind']}</span>{badge(p['status'])}</div><h3>{p['name']}</h3><p class="place-time">{p['time']}</p><details><summary>住所・駐車場・くわしい案内 <span>＋</span></summary><div class="place-detail"><p>{p['intro']}</p>{p['detail']}</div></details><div class="place-links">{place_links(p)}</div></article>'''
...
stamps_html=make_stamps().replace('</section>', make_postcard_studio()+'</section>')
museum_invitation=build_museum()
...
<header class="site-header"><a class="brand" href="#top" aria-label="ふくいのしおり 表紙へ"><span class="brand-mark">福</span><span>ふくいのしおり<small>FUKUI TRAVEL JOURNAL</small></span></a><nav aria-label="メインナビ"><a href="#timetable">3日間の旅</a><a href="#route">地図</a><a href="#places">場所の案内</a><a href="museum.html">資料館</a><a href="#preparation">旅の準備</a></nav><span class="header-date">2026.09.21 — 23</span></header>
<main id="top">
<section class="cover" aria-label="旅の表紙"><img class="cover-photo" src="assets/tojinbo.webp" alt="青い日本海に面する東尋坊の柱状節理の断崖" width="1800" height="1200" fetchpriority="high"><div class="cover-shade"></div><div class="cover-content"><p class="eyebrow">FUKUI / SEPTEMBER 2026</p><h1>ふくい、<br>余白の<span>三日間。</span></h1><p class="cover-copy">まだ知らない福井を、<br>ふたりのペースで。</p><div class="cover-actions"><a class="cover-cta" href="#timetable">旅をひらく <span>↗</span></a><a class="cover-map" href="#route">地図で見る ↗</a></div></div><div class="cover-bottom"><span>2026 <strong>09.21 — 09.23</strong></span><span>TOKYO → FUKUI<br>2 NIGHTS / 3 DAYS</span><span class="photo-place">東尋坊 · 日本海</span></div><a class="cover-seal" href="#stamps"><span>ふたり旅</span><strong>2泊3日</strong><small>FUKUI, JAPAN</small></a></section>

<section class="now-section section-wrap" id="now" aria-labelledby="now-heading"><div class="now-label"><span class="status-dot"></span><span id="journey-state">BEFORE THE JOURNEY</span><h2 id="now-heading">旅のはじまりまで</h2></div><div class="now-main" aria-live="polite"><p id="now-kicker">9月21日（月） 東京駅から出発</p><h3 id="now-title">06:16 東京発 → 09:12 福井着</h3><p id="now-note">かがやき501号。東京駅には05:45頃に到着を。</p><a id="now-fixed" href="#schedule-1" hidden><small>次の固定予定</small><span></span></a></div><a class="round-link" href="#journey" id="now-link">旅程へ <span>↗</span></a></section>
{{POCKETS}}
{{SUMMARY}}
...
<section class="places-section" id="places"><div class="section-wrap"><div class="section-heading"><div><p class="eyebrow">GOOD TO KNOW</p><h2>場所の案内。</h2></div><div class="place-filters" aria-label="訪問日で絞り込む"><button type="button" data-filter="all" aria-pressed="true">すべて</button><button type="button" data-filter="1" aria-pressed="false">Day 1</button><button type="button" data-filter="2" aria-pressed="false">Day 2</button><button type="button" data-filter="3" aria-pressed="false">Day 3</button></div></div><details class="place-directory" id="place-directory"><summary><span>観光・食事・宿・交通</span><span>15か所の案内をひらく ＋</span></summary><div class="place-grid">{{PLACES}}</div></details></div><details class="food-drawer" id="food"><summary>食と日本酒の楽しみ ＋</summary><div class="food-section"><div class="section-heading"><div><p class="eyebrow">04 / TASTE OF FUKUI</p><h2>おいしい記憶を、少しずつ。</h2></div><p>海鮮、会席、郷土の味。<br>その日の一杯を、ふたりで。</p></div><div class="food-grid"><a class="food-note" href="#place-hasegawa"><span class="food-index">一</span><p class="eyebrow">DAY 1 / あわら温泉</p><h3>湯上がりに、<br>結会席。</h3><p>福井の食材と焼きしゃぶ。<br>料理に合う地酒は、宿で相談。</p><span class="food-link">長谷川の案内 ↗</span></a><a class="food-note" href="#place-lunch"><span class="food-index">二</span><p class="eyebrow">DAY 2 / 三国と福井</p><h3>旬の甘えび。<br>夜は、福井酒。</h3><p>昼は三国港甘えび旬彩御膳が候補。<br>20:45から煙やで郷土の味を。</p><span class="food-link">三国の昼食候補 ↗</span></a><a class="food-note" href="#place-mizumoto"><span class="food-index">三</span><p class="eyebrow">DAY 3 / 一乗谷と駅</p><h3>おろしそばと、<br>旅を結ぶ一杯。</h3><p>昼はそばを軽やかに。車を返したら、<br>みずもとで気になる銘柄を。</p><span class="food-link">角打ちの案内 ↗</span></a></div><p class="sake-note">二人でお酒を楽しむのは、初日の旅館・Day 2の駐車後・最終日の車の返却完了後に。ESHIKOTOでは運転する人は試飲をしません。</p></div></details></section>

{{STAMPS}}
<section class="prep-section" id="preparation"><div class="section-wrap"><div class="section-heading"><div><p class="eyebrow">PACK LIGHT, TRAVEL HAPPY</p><h2>出発前のポケット。</h2></div><span id="check-progress" class="check-progress">準備チェック</span></div><p class="storage-hint">チェックとメモはこの端末に保存されます。二人の端末間では共有されません。</p><p class="storage-warning" id="storage-warning" hidden>このブラウザでは保存できません。入力はページを閉じると失われます。</p><div class="prep-grid"><details class="prep-tasks prep-drawer"><summary>手配・予約の確認 <span>＋</span></summary><div><p class="task-note">一乗谷ガイドは通常申込期限後。手配可否を早めに確認。</p>{{PREP}}</div></details><div class="packing"><details class="prep-drawer"><summary>旅のかばん <span>＋</span></summary><div>{{PACKING}}</div></details><label class="memo-label" for="trip-memo">ふたり旅のメモ</label><textarea id="trip-memo" rows="5" placeholder="気になった日本酒、買いたいお土産、忘れたくないこと。"></textarea><p class="memo-status" id="memo-status" role="status">この端末だけに保存</p><button class="text-button" id="export-memo" type="button">メモとチェックを保存用ファイルに書き出す ↓</button></div></div></div></section>
<section class="section-wrap archive-section" id="reference"><div class="section-heading"><div><p class="eyebrow">THERE IS MORE TO DISCOVER</p><h2>旅の資料館。</h2></div></div>{{MUSEUM}}<div class="practical-library-heading"><h3>実用の書庫。</h3><button class="outline-button" type="button" id="print-guide">しおりを印刷 ↗</button></div><details class="archive" id="reservations"><summary><span>01</span> 交通・宿泊・予約一覧 <b>＋</b></summary><div class="archive-body"><p>金額は2名分。予約済み固定費186,951円、支払済み確認分159,132円。手織り体験と煙やは固定費に含みません。</p><div class="table-scroll" tabindex="0" role="region" aria-label="予約一覧"><table><thead><tr><th>内容</th><th>予約内容</th><th>金額</th><th>状況</th></tr></thead><tbody>{{RESERVATIONS}}</tbody></table></div><p>新幹線は往復とも普通車指定席・2名並び席。号車・座席・乗車方法・予約番号・キャンセル条件は、手元の予約確認画面で確認してください。チケットQRコードはこの公開ページには保存しません。</p></div></details><details class="archive"><summary><span>02</span> ふたり分の予算 <b>＋</b></summary><div class="archive-body"><div class="budget-head"><span>土産を除く中央見積もり / 2名</span><strong>¥249,691</strong><p>土産込みで約26万〜27万円。余裕を持つなら27万〜28万円程度。</p></div><div class="table-scroll" tabindex="0" role="region" aria-label="予算明細"><table><thead><tr><th>項目</th><th>金額</th><th>備考</th></tr></thead><tbody>{{BUDGET}}</tbody></table></div><p>保存協会の4,000円ガイドの場合は1,500円追加。カフェやガイドを利用しなければ該当分を差し引きます。持ち帰る日本酒は土産代です。</p></div></details><details class="archive" id="rain-plan"><summary><span>03</span> 雨・渋滞・疲れたとき <b>＋</b></summary><div class="archive-body rain-grid">{{RAIN}}</div></details><details class="archive"><summary><span>04</span> オフラインとホーム画面への追加 <b>＋</b></summary><div class="archive-body"><p id="offline-status" role="status">オフライン保存の状態を確認しています。</p><p>一度オンラインでしおりの保存が完了すると、旅程・住所・電話番号・主要な写真をオフラインでも読めます。このしおり内の地図・タイムテーブル・資料館の読みものとクイズもオフラインで見られます。Google マップの経路検索・施設の公式サイトには通信が必要です。</p><p>iPhoneはSafariの共有メニューから「ホーム画面に追加」。Androidはブラウザのメニューから「ホーム画面に追加」または「アプリをインストール」を選びます。</p><button id="install-app" class="outline-button" type="button" hidden>ホーム画面に追加する ↗</button><p>予約画面やチケット、必要な地図は別途保存してください。保存データはブラウザの設定や空き容量により消える場合があります。</p></div></details></section>
